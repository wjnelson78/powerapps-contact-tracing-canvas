...
$internalList = Get-Content '<<Path>>\check-in-list.json' -Raw | Add-SPOSiteScript -Title "Internal Check-in List" -Description "Check-in list for internal users" | select Id
$externalList = Get-Content '<<Path>>\external-check-in-list.json' -Raw | Add-SPOSiteScript -Title "External Check-in List" -Description "Check-in list for external users" | select Id
